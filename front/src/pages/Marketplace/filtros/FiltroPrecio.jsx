...
const FiltroPrecio = ({ Planificaciones, setPlanificaciones }) => {
  ...
  const FiltradoPrecio = () => {
    const FiltradoPlanificaciones = Planificaciones.filter(
      (Planificaciones) =>
        Planificaciones.price >= preciominimo &&
        Planificaciones.price <= preciomaximo
    );
  ...
  return (
    <form
      ...
      className="max-w-[300px] mb-5 m-2"
    >
      ...
      <input
        ...
        value={preciomaximo}
        min={preciominimo}
        onChange={PrecioMaximo}
        className="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
      />

      <button
        onClick={FiltradoPrecio}
